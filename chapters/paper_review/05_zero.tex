...
\caption{
	...
	\midrule
	\cite{liu_completely_2018}& Audio word2vec~\cite{wang_segmental_2018} & Oracle & Phoneme & \edit{Adversarial Training~\cite{gulrajani_improved_2017}} & - & TIMIT (PER): 63.6\%  \\ %Submitted on 1 Apr 2018
	\midrule
	\cite{chung_unsupervised_2018} & Speech2vec~\cite{chung_speech2vec_2018} & BES-GMM~\cite{kamper_segmental_2017} &  Word2Vec & \edit{Adversarial Training~\cite{conneau_word_2018}} & Self-training & SWC (Acc): 10.9\%  \\ %Submitted on 18 May 2018
	...
	\cite{chung_unsupervised_2019a} & \tabincell{l}{Speech2vec\\(English)} & Oracle & \tabincell{c}{Word2Vec\\(French)} & VecMap~\cite{artetxe_robust_2018} & \tabincell{l}{LM rescore,\\sequence DAE} & ST (BLEU): 10.8\%   \\ %[Submitted on 4 Nov 2018]
	\midrule
	
	\cite{yeh_unsupervised_2019} & MFCC & GAS~\cite{wang_gate_2017} & Phoneme & Empirical-ODM~\cite{liu_unsupervised_2017} & Self-training &  TIMIT (PER):  41.6\%  \\ %Submitted on 23 Dec 2018
	\midrule

	\cite{chen_completely_2019} & MFCC & GAS  & Phoneme & \edit{Adversarial Training~\cite{gulrajani_improved_2017}} & Self-training &  TIMIT (PER):  33.1\%   \\
	\midrule

	\cite{baevski_unsupervised_2021} & Wav2vec 2.0~\cite{baevski_wav2vec_2020} & $k$-means & Phoneme &\edit{Adversarial Training~\cite{gulrajani_improved_2017}} & Self-training & \tabincell{l}{TIMIT (PER):  18.6\%, \\ LibriSpeech (WER): 5.9\%}   \\
	\midrule
	
	\textcolor{black}{\cite{klejch_deciphering_2022}} & \textcolor{black}{\tabincell{l}{Universal Phone \\ Recogniser}} & - & \textcolor{black}{Grapheme} & \textcolor{black}{Decipherment~\cite{ravi_deciphering_2011}} & \textcolor{black}{Self-training} & \textcolor{black}{\tabincell{l}{GlobalPhone: 32.5\% to just 1.9\% \\ worse than supervised models} }  \\ %ubmitted on 12 Nov 2021 %m 32.5% to just 1.9% absolute worse than the equivalent fully supervised models trained on the same data
	\midrule
	\textcolor{black}{\cite{liu_endtoend_2023}} & \textcolor{black}{Wav2vec 2.0~\cite{baevski_wav2vec_2020} }& - & \textcolor{black}{Phoneme} & \textcolor{black}{Adversarial Training~\cite{gulrajani_improved_2017}} & \textcolor{black}{Self-training} & \textcolor{black}{LibriSpeech (WER): 6.3\%}   \\
	\midrule

	\textcolor{black}{\cite{liu_endtoend_2023}} & \textcolor{black}{Wav2vec 2.0~\cite{baevski_wav2vec_2020}} & - & \textcolor{black}{Grapheme} & \textcolor{black}{Adversarial Training~\cite{gulrajani_improved_2017}} & \textcolor{black}{Self-training} & \textcolor{black}{LJSpeech (WER): 64.0\%}  \\
...
Adversarial training~\cite{goodfellow_generative_2014,arjovsky_wasserstein_2017, gulrajani_improved_2017} is one common way to learn such a 
mapping function. 
...
\edit{
There is a large amount of work using gradient penalty in the objective of training discriminators~\cite{liu_completely_2018,chen_completely_2019,baevski_unsupervised_2021,liu_endtoend_2023}, which is inspired by Improved Wasserstein Generative Adversarial Network (WGAN)~\cite{gulrajani_improved_2017}.
}
\textcolor{black}{
...
phoneme-level
segments~\cite{liu_completely_2018,chen_completely_2019,yeh_unsupervised_2019}. 
...
Wav2vec-U~\cite{baevski_unsupervised_2021} selects the input feature from different layers of wave2vec~2.0~\cite{baevski_wav2vec_2020}.
The selection criterion is based on analysis of the phonetic information in each layer.
\textcolor{black}{
If a universal phone recognizer trained from a diverse set of languages is available, it is another way to transcribe speech into phone-level tokens~\cite{klejch_deciphering_2022}.}
Another series of work is to transform a word into a word embedding. 
\cite{chung_unsupervised_2018,chung_unsupervised_2019a} use adversarial training to map the word-level
...
\cite{liu_completely_2018} is the first work to successfully achieve unsupervised
phoneme recognition by first clustering phoneme-level
embeddings~\cite{chung_audio_2016,wang_segmental_2018} into a set of
tokens and then using GAN to learn the mapping relationship between tokens and
phonemes. 
However, in \cite{liu_completely_2018}, oracle phoneme segmentation is still
required to get reasonable performance. 
\cite{chen_completely_2019,yeh_unsupervised_2019} uses gate activation signals
(GAS)~\cite{wang_gate_2017} to get the phoneme-level segmentation, which is
unsupervised. 
%However, the loss term in~\cite{yeh_unsupervised_2019} includes an empirical average over all segments inside a logarithmic function, which can be biased if we sample this empirical average by a mini-batch average. \cite{liu2017unsupervised} therefore use an extremely large batch size during training to reduce the biasing problem.
%These problems are handled by another work~\cite{chen_completely_2019}.
%\cite{chen_completely_2019} removes the quantization step.
%Compared to~\cite{yeh_unsupervised_2019}, the discriminator in~\cite{chen_completely_2019} considers all possible information from the text dataset, not limited to the n-gram local statistics.
%Besides, \cite{chen_completely_2019} works well with a reasonable batch size, which makes it feasible when the computational resource is limited.
Wav2vec-U~\cite{baevski_unsupervised_2021} selects the input feature from
different layers of wave2vec 2.0~\cite{baevski_wav2vec_2020}, a self-supervised
model.
...
%In \cite{chung_unsupervised_2018, chung_unsupervised_2019a}, the word boundaries are also generated automatically with speech only. 
%However, \cite{chung_unsupervised_2018, chung_unsupervised_2019a} need oracle boundaries, therefore not completely unsupervised as the framework proposed in this paper.
...
Wav2vec-U~\cite{baevski_unsupervised_2021} achieved state-of-the-art results at the time, which suggests that representation learning is
...
\textcolor{black}{
And wav2vec-U 2.0~\cite{liu_endtoend_2023} further removes the requirement of the segmentation stage, so the unsupervised ASR model can be learned in an end-to-end style.}
...
speech and text were different~\cite{lin_analyzing_2022}.
...
data.
Therefore, wav2vec-U~\cite{baevski_unsupervised_2021} develops a new approach to
...
The ASR-TTS framework can jointly optimize both ASR and TTS \edit{without using paired data}~\cite{tjandra_listening_2017,hori_cycleconsistency_2019,wang_improving_2020}.
A speech chain~\cite{tjandra_listening_2017,tjandra_machine_2018} is one 
...
extensions of this study~\cite{hori_cycleconsistency_2019,tjandra_endtoend_2019,baskar_semisupervised_2019}
implement the joint training of both ASR and TTS parameters using 
REINFORCE~\cite{williams_simple_1992} and straight-through estimators.
...
model~\cite{jelinek_statistical_1997}, a weighted finite state 
transducer~\cite{mohri_weighted_2002}, or shallow 
fusion~\cite{gulcehre_using_2015,chorowski_better_2017}.
However, the progress of TTS systems boosted by deep 
learning~\cite{oord_wavenet_2016,shen_natural_2018} has inspired another interesting and
...
variations~\cite{li_training_2018,ueno_multispeaker_2019,rosenberg_speech_2019,laptev_you_2020,huang_using_2020}.
...
features~\cite{hayashi_backtranslationstyle_2018} and phoneme 
features~\cite{renduchintala_multimodal_2018,masumura_phonemetographeme_2020}.
This approach is similar to the back-translation technique developed in \edit{neural machine translation}~\cite{sennrich_improving_2016}.
...
\begin{itemize}
	 \item \edit{Zero Resource Speech Challenge 2015~\cite{versteegh_zero_2015} mainly focused on building an acoustic model without using any
    ...
    Both tracks were based on the English and Xitsonga languages.
	 \item The Zero Resource Speech Challenge 2017~\cite{dunbar_zero_2017} focused on 
	 ...
	 \item The Zero Resource Speech Challenge 2019~\cite{dunbar_zero_2019} extended a goal of
	 ...
	 \item The Zero Resource Speech Challenge 2020~\cite{dunbar_zero_2020} was based on two
	 ...
	 \item The Zero Resource Speech Challenge 2021~\cite{nguyen_zero_2020}, the latest
...
models~\cite{ondel_variational_2016,heck_feature_2017}, deep 
autoencoders~\cite{tjandra_vqvae_2019,chorowski_unsupervised_2019}, and incorporation of
neural-network-based TTS/VC techniques~\cite{vanniekerk_vectorquantized_2020} to
self-supervised learning~\cite{maekaku_speech_2021}.
...
labels~\cite{nguyen_generative_2022,polyak_speech_2021,kharitonov_textfree_2021,kreuk_textless_2022}. 
...
The generative spoken language model (GSLM)~\cite{nguyen_generative_2022} utilizes discrete
...
model and generate
dialogues~\cite{nguyen_generative_2022}.\footnote{https://speechbot.github.io/dgslm/}
%\cite{}. 
Given its flexibility in modeling spoken content, the GSLM has been further extended
to jointly model content and prosody~\cite{kharitonov_textfree_2021}. This prosodic-GSLM model
...
%\cite{}. 
Going one step further, \cite{kreuk_textless_2022} used a speech emotion
...
spoken question answering~\cite{lin_dual_2022}, speech separation~\cite{shi_discretization_2022}, TTS~\cite{hayashi_discretalk_2020}, and speech-to-speech
translation~\cite{lee_textless_2022}.
