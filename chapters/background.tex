...
OOD detection is the task of identifying data that is unlikely to originate from the distribution of the training data and, in the context of neural networks, the task dates back to at least 1994 \cite{bishop_novelty_1994}. 
...
Many approaches for OOD detection have been proposed in the literature. One way to categorize the different approaches is based on whether the method estimates a conditional probability distribution $p(y|\xb)$ over some target variable given the input, or a probability distribution $p(\xb)$ over the input itself. We might refer to the former as \emph{supervised} OOD detection, and the latter as \emph{unsupervised} OOD detection \cite{graham_denoising_2023,liu_unsupervised_2023a}. Supervised OOD detection tries to assess whether the model's prediction $p(y|\xb)$ should be trusted for a given input $\xb$, whereas unsupervised OOD detection judges whether the input $\xb$ should be trusted and used at all. 
...
%
In the supervised setting the supervision can come from different sources depending on how much we know, or assume to know, about the OOD data. 
The most informed approaches assume we have access to representative OOD data such that we can modify the training procedure to explicitly model the OOD data. 
Such approaches have achieved high performance, but are inherently limited by the availability of such OOD data and how well it covers the large variation in OOD data \cite{hendrycks_deep_2019,ruff_deep_2020}.  
As we discussed earlier, for many modern applications of machine learning, the data is high dimensional and complex which makes sufficiently sampling the space of out-of-distribution data intractable. 

A number of less informed methods do not require access to actual OOD data but instead use synthetic OOD data. 
...
For instance, \textcite{ren_likelihood_2019} propose a number of baselines including training a binary classifier to distinguish between original and perturbed in-distribution data, using an additional class trained to predict perturbed in-distribution data, and training the predicted class distribution to output uniform distribution for perturbed in-distribution inputs \cite{dhamija_reducing_2018}. 
In ODIN \cite{liang_enhancing_2018}, the authors propose to add gradient-based, input-dependent noise to the inputs of a pre-trained classifier and use the calibrated maximum class probability of the predictive distribution $p(y|\xb)$ to evaluate whether the example is in-distribution or not. This method does not require retraining the classifier and generally improves on the previous, simple methods. 
A different approach generates OOD inputs using a GAN \cite{lee_training_2018}. 
As the most informed approaches, this group of methods is also limited by the intractability of sampling the space of OOD data.

The least informed supervised OOD detection methods do not require instantiations of real or synthetic OOD data. 
A generalized version of ODIN \cite{hsu_generalized_2020} removes the need for simulating OOD data by following a probabilistic motivation and computing output logits as a fraction between two carefully designed network branches. The authors note that most current methods make a closed world assumption and implicitly condition on the in-domain $d_\text{in}$ in the form of the predictive distribution $p(y|\xb, d_\text{in})$. With this observation, the authors decompose the $p(y|\xb, d_\text{in})$ into a joint class-domain probability and a domain probability,
\begin{equation}
    p(y|\xb,d_\text{in}) = \frac{p(y, d_\text{in}|\xb)}{p(d_\text{in}|\xb)} \enspace .
\end{equation}

\todo[inline]{This here is WIP}

One particularly simple approach uses the maximum class probability of $p(y|\xb)$ to evaluate whether the example is in-distribution or not by noting that it tends to be larger for correctly classified examples \cite{hendrycks_baseline_2017}. Another simple method proposes that a high entropy of $p(y|\xb)$ indicates an OOD $\xb$ \cite{ren_likelihood_2019} while \textcite{hendrycks_scaling_2019} suggested using the maximum logit value and KL-divergence matching. 


\textcite{lee_simple_2018} fit a multivariate Gaussian distribution to the activations of the penultimate layer of a pre-trained classifier and use the Mahalanobis distance to this distribution to evaluate whether inputs are in-distribution or not. 
\textcite{lakshminarayanan_simple_2017} propose to use an ensemble of independently trained classifiers to discriminate between in-distribution and OOD data by evaluating the agreement between the classifiers. 
\textcite{huang_importance_2021} compute the gradient of the KL-divergence of the predictive distribution $p(y|\xb)$ to a uniform distribution noting that the magnitude of gradients is higher for in-distribution data than for OOD data. 

Other approaches  \cite{devries_learning_2018, huang_mos_2021, zaeemzadeh_outofdistribution_2021}


\cite{liu_energy-based_2020} propose to use an energy-based model to detect OOD data. UNSUPERVISED?
...
\cite{lakshminarayanan_simple_2017, liang_enhancing_2018 , huang_importance_2021, wang_vim_2022} % lee_simple_2018, hendrycks_baseline_2017
