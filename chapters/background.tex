...
\todo[inline]{Maybe this section should come after the VAE background?}
% 
...
\todo[inline]{Briefly hold the readers hand at actualize the importance of OODD.}
...
\paragraph{Related tasks} OOD detection bears many similarities with anomaly detection, novelty detection, open set recognition, and outlier detection \parencite{yang_generalized_2022}. 
...
\todo[inline]{Does the previous sentence belong here or in a different paragraph? It is not really using the latent space.}
...
\moretodo[inline]{Briefly hold the readers hand and tell them why VAEs are relevant to the overarching project.}
...
This precludes training with commonly used methods such as expectation maximization and exact maximum likelihood estimation and while MCMC methods are usually too computationally expensive for large neural networks, variational inference provides a scalable alternative \parencite{jordan_introduction_1999}. 
...
By estimating the posterior distribution $p(\zb|\xb)$ with a variational approximation $q(\zb)$ we can derive a lower bound on the otherwise intractable marginal log-likelihood $\log p_\theta(\xb)$.
%
 \begin{align} \label{eq:variational-autoencoder-elbo}
    \log p_\thetab(\xb)
    &= \log \int p_\thetab(\xb,\zb),\textrm{d}\zb \nonumber \\
    &= \log \int q(\zb) \frac{p_\thetab(\xb,\zb)}{q(\zb)}\,\textrm{d}\zb \nonumber \\
    &\geq \int q(\zb) \log \left( \frac{p_\thetab(\xb,\zb)}{q(\zb)} \right) \,\textrm{d}\zb \nonumber \\
    &\geq \mathbb{E}_{q(\zb)} \left[ \log \frac{p_\thetab(\xb,\zb)}{q(\zb)} \right] \equiv \mathcal{L}_{}(\xb;q) \enspace .
...
\end{align}
%
Since we can evaluate the generative model $p_\thetab(\xb,\zb)$ and are free to define $q(\zb)$ as we please, this bound is tractable. 
However, finding an approximate posterior distribution $q(\zb)$ that is ...

\moretodo[inline]{Glue this together..}

Instead, VAEs amortize the cost of variational inference by parameterizing $q_\phib(\zb|\xb)$ as a single neural network. This makes it possible to efficiently compute approximate posterior distributions for any input $\xb$. The approximate posterior can also be used to compute a lower bound on the marginal distribution $p_\thetab(\xb)$ via Jensen's inequality, the so-called evidence lower bound (ELBO), 
%
\begin{equation}
    \log p_\thetab(\xb) \geq \mathbb{E}_{q_\phib(\zb|\xb)} \left[ \log \frac{p_\thetab(\xb,\zb)}{q_\phib(\zb|\xb)} \right] \equiv \mathcal{L}_{}(\xb;\thetab,\phib) \enspace .
\end{equation}
%
Although analytically evaluating the ELBO is still intractable, it can be approximated by sampling and used to train VAEs. We will return to this after examining the ELBO in more detail.
...
By amortizing the variational inference we introduce a gap compared to the alternative of fitting a posterior per data point, $q^{*}_{\xb}(\zb)$. This is referred to as the amortization gap and can be reduced by using large capacity amortized posteriors. 
In case the true posterior does not belong to the model class of the variational family of posteriors that we fit, the inference gap cannot be reduced to zero and the remaining gap is referred to as the approximation gap. 
If the inference gap is closed, the maximum of the ELBO corresponds to the maximum of the maringa
Finally, the generative model 

\moretodo[inline]{Finish this with the parsimony gap}
...
\moretodo[inline]{Maybe add a small graphic illustrating the gaps.}
...
\paragraph{Sampling}
Since we have freedom to select the approximate posterior, we can choose it such that it is easy to sample and then use that property to form a Monte Carlo estimate of the ELBO,
% 
...
Compared to the estimator in \cref{eq:variational-autoencoder-elbo-sampling-expanded,eq:variational-autoencoder-elbo-sampling-KL} which 

\moretodo[inline]{Derive and introduce the IWAE bound.}
...
\begin{equation}
    ...
    q_\phib(\zb|\xb) = q_\phib(\zb_L|\xb)\prod_{i=1}^{L-1} q_\phib(\zb_{i}|\zb_{i+1}) \ .
\end{equation}
...
\moretodo[inline]{Revise according to change of review papers.}
...
For a deeper comparison of VAEs and self"=supervised methods for speech processing, we refer the reader to our paper \textcite{borgholt_brief_2022} included in \cref{chp:paper-brief}. 
\Cref{app:paper-review} provides a comprehensive introduction to and review of self"=supervised learning in speech recognition. 
