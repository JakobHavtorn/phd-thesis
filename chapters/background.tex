...
\moretodo[inline]{Briefly hold the readers hand at actualize the importance of OODD.}
...
\moretodo[inline]{Does the previous sentence belong here or in a different paragraph? It is not really using the latent space.}
...
\begin{align}
    ...
    &\leq \log \mathbb{E}_{\zb_{s}\sim q_\phib(\zb|\xb)}\left[ \frac{1}{S}\sum_{s=1}^{S} w_\thetab(\xb,\zb_{s}) \right] \enspace . \nonumber \\
    &= \log p(\xb) \enspace .
\end{align}
%
...
With $S=1$, the estimator is equivalent to the regular ELBO \cref{eq:variational-autoencoder-elbo} and so is always as tight or tighter than the ELBO \parencite{burda_importance_2016}.
...
\begin{equation}
    ...
    q_\phib(\zb|\xb) = q_\phib(\zb^{(L)}|\xb)\prod_{i=1}^{L-1} q_\phib(\zb^{(i)}|\zb^{(i+1)}) \ .
\end{equation}
