...
%
 \begin{align} \label{eq:variational-autoencoder-elbo}
    ...
    % &\geq \mathbb{E}_{q_\phib(\zb|\xb)} \left[ \log \frac{p_\thetab(\xb|\zb)p(\zb)}{q_\phib(\zb|\xb)} \right] \equiv \mathcal{L}_{}(\xb;\thetab,\phib) \enspace .
\end{align}
%
...
% Since the ELBO is computed as an expectation under the variational approximation to the posterior, $q_\phib(\xb|\zb)$, we will seek to rewrite the log-marginal likelihood into such an expectation, .
%
\begin{align}
    \log p_\thetab(\xb) 
