...
One way to categorize the different approaches is based on whether the underlying model is a classifier that estimates a conditional probability distribution $p(y|\xb)$ over some target variable given the input, or a model that learns a probability density $p(\xb)$ over the input itself. We might refer to the former as \emph{supervised} OOD detection, and the latter as \emph{unsupervised} OOD detection \cite{graham_denoising_2023,liu_unsupervised_2023a}, although it is important to note that this distinction relates only to the availability of some target value, $y$ - not whether OOD data is available for supervision\footnote{
...
For high-dimensional data, the most successful unsupervised models for OOD detection are deep autoencoders \cite{hinton_reducing_2006} and deep generative models. 
...
There exist at least six main classes of deep generative models: variational autoencoders \cite{kingma_autoencoding_2014,rezende_stochastic_2014}, flow-based models \cite{dinh_nice_2015,kingma_glow_2018}, autoregressive models \cite{oord_conditional_2016,oord_wavenet_2016}, diffusion models \cite{sohl-dickstein_deep_2015,ho_denoising_2020}, energy-based models \cite{lecun_tutorial_2006,salakhutdinov_efficient_2010,grathwohl_your_2020}, and generative adversarial networks \cite{goodfellow_generative_2014,arjovsky_wasserstein_2017}.
...
However, compared to deep generative models, these methods are not well suited for high-dimensional data such as images, text or audio, and have had little direct impact on the recent work on OOD detection. 
...
% \cite{
%     X sakurada_anomaly_2014                 encode and decode data with autoencoders.
%     X xia_learning_2015                     encode and decode data with autoencoders.
%     zhou_anomaly_2017                     encode and decode data with autoencoders.
%     zong_deep_2018                        encode and decode data with autoencoders.
%     lyudchik_outlier_2016                 encode and decode data with autoencoders.
%     chen_outlier_2017                     encode and decode data with an ensemble of autoencoders.
%     schlegl_unsupervised_2017             perform GAN inversion for a data point, and evaluate its reconstruction error and discriminator confidence under the inverted latent variable.
%     li_madgan_2019                        perform GAN inversion for a data point, and evaluate its reconstruction error and discriminator confidence under the inverted latent variable.
%     graham_denoising_2023                 leverages diffusion models to reconstruct images at varied diffusion steps, while we mask and inpaint an image repeatedly with fixed steps.
%     liu_unsupervised_2023a                LMD lifts an image off its original manifold by corrupting it, and maps it towards the in-domain manifold with a diffusion model. For an out-of-domain image, the mapped image would have a large distance away from its original manifold, and LMD would identify it as OOD accordingly.
% }
%
A number of methods derive the OOD score from a reconstruction error. 
Among the first methods are \textcite{sakurada_anomaly_2014} and \textcite{xia_learning_2015} who use deep autoencoders to reconstruct the input and use the reconstruction error as the OOD score. 
% In \textcite{zhou_anomaly_2017} 

\paragraph{Representation-based}
% https://arxiv.org/pdf/2302.10326.pdf
% \cite{
%     denouden_improving_2018,              leverages the latent variables of an autoencoder, and evaluates the Mahalanobis distance in the latent space along with the data reconstruction error
%     ahmadian_likelihoodfree_2021,         extracts low-level features from the encoder of an invertible generative model.
%     hendrycks_using_2019,                 learn a representation over the in-domain data through self-supervised training
%     bergman_classificationbased_2020,     learn a representation over the in-domain data through self-supervised training
%     tack_csi_2020,                        learn a representation over the in-domain data through self-supervised training
%     sehwag_ssd_2021,                      learn a representation over the in-domain data through self-supervised training
%     xiao_we_2021                          further shows that one can instead use a strong pretrained feature extractor while maintaining comparable performance.
% }
%
