...
% No OOD data, logits
Although some works use the maximum softmax probability as a score for OOD detection \cite{hendrycks_deep_2019,ren_likelihood_2019}, several works have noted that it is not generally reliable \cite{hendrycks_scaling_2022,liu_energy-based_2020}. 
...
% A variational autoencoder (VAE) \cite{kingma_autoencoding_2014,rezende_stochastic_2014} is a type of deep generative model and an instance of a latent variable model. 
In the following, we will introduce VAEs starting from the definition of the group of latent variable models to which they belong. We then derive the maximum likelihood objective used to train VAEs, discuss its different components, and highlight some central challenges. Finally, we discuss some of the most important extensions to VAEs made model speech data. 
...
\end{equation}
%
In latent variable models such as Gaussian mixture models \cite{dempster_maximum_1977}, hidden Markov models \cite{rabiner_tutorial_1989}, and probabilistic principal component analysis \cite{tipping_probabilistic_1999}, the joint distribution is factorized as $p(\xb,\zb) = p(\xb|\zb)p(\zb)$.
...
In these cases, approximate methods such as variational inference \cite{jordan_introduction_1999} and Markov Chain Monte Carlo (MCMC) \cite{mohamed_monte_2019} are used to compute the marginal and posterior distributions.


\paragraph{Variational autoencoders} 
To model high-dimensional data, \textcite{kingma_autoencoding_2014,rezende_stochastic_2014} propose to parameterize the generative model $p_\thetab(\xb|\zb)$ with a neural network with parameters $\thetab$. Since this makes the marginal and posterior distributions intractable, both training and inference require approximate methods. 
...
Specifically, VAEs amortize the cost of inference by parameterizing $q_\phib(\zb|\xb)$ as a single neural network which makes it possible to efficiently compute approximate posterior distributions for any input $\xb$. With the approximate posterior, we can also compute the so-called evidence lower bound (ELBO) on the marginal distribution $p(\xb)$, 
%
...
Ideally, we want to use maximum likelihood estimation to maximize the ELBO with respect to the parameters $\thetab$ and $\phib$ and jointly optimize the generative model $p_\thetab(\xb|\zb)$ and the variational approximation $q_\phib(\zb|\xb)$. 
Computing the gradient of the ELBO with respect to the parameters of the generative model, $\thetab$, is straightforward,
%
\begin{equation}
...
\end{equation}
%
More troublesome is the gradient with respect to the parameters of the variational approximation, $\phib$. Here the parameters occur in the expectation itself and it is not possible to move the gradient inside the expectation. 
However, by reparameterizing the latent variable $\zb$ as a deterministic function of the input $\xb$ and a random variable $\epsilonb\sim p(\epsilonb)$, $\zb = g_\phib(\xb,\epsilonb)$, we can write the gradient as,
%
\begin{equation}
...
\begin{align} \label{eq:variational-autoencoder-elbo-factorized}
    \log p(\xb) \geq \mathcal{L}_{\thetab,\phib}(\xb)
            ...
            &= \mathbb{E}_{q_\phib(\zb|\xb)}\left[\log p_\thetab(\xb|\zb)\right] - D_\text{KL}\left( q_\phib(\zb|\xb) \,||\, p(\zb) \right)
\end{align}
%
This reveals an interpretation of the ELBO as the negative KL divergence between the variational approximation $q_\phib(\zb|\xb)$ and the prior distribution $p(\zb)$ plus the expected log-likelihood of the data under the generative model $p_\thetab(\xb|\zb)$. 

The training of VAEs is usually performed by stochastic gradient descent on the ELBO. This method samples a mini-batch of data points $\xb$ from the empirical training data distribution $\hat{p}(\xb)$ and then evaluates the gradient of the ELBO with respect to the parameters $\thetab$ and $\phib$ on the mini-batch. 
To gain some insight into the objective that we are optimizing, we will see if we can exploit the expectation over the data distribution $\hat{p}(\xb)$ to form a factorization similar to \cref{eq:variational-autoencoder-elbo-factorized} but with the aggregated posterior $q_\phib(\zb) = \int q_\phib(\zb|\xb)\hat{p}(\xb)\,\mathrm{d}\xb$ in the KL-divergence. It will turn out that we can indeed factor the ELBO into such a KL-divergence, a reconstruction term, and a third term \cite{tomczak_trouble_2022}. For ease of notation, we will let $q(\xb) = \hat{p}(\xb)$ so that $q_\phib(\zb,\xb) = q_{\phib}(\zb|\xb)\hat{p}(\xb)$. 
%
\begin{align}
    \mathbb{E}_{\hat{p}(\xb)}\left[ \log p(\xb) \right] 
    &= \mathbb{E}_{\hat{p}(\xb)}\left[ \log \int p_{\thetab}(\xb | \zb)\ p(\zb) \, \mathrm{d}\zb \right] \nonumber \\
    &\geq \mathbb{E}_{\hat{p}(\xb)}\left[ \mathbb{E}_{q_{\phib}(\zb|\xb)}\left[ \log p_{\thetab}(\xb | \zb) + \log p(\zb) - \log q_{\phib}(\zb | \xb) \right] \right] \nonumber \\
    &= \mathbb{E}_{q_{\phib}(\zb|\xb)\hat{p}(\xb)}\left[ \log p_{\thetab}(\xb | \zb) + \log p(\zb) - \log q_{\phib}(\zb | \xb) \right] \nonumber \\
    &= \mathbb{E}_{q_{\phib}(\zb,\xb)}\left[ \log p_{\thetab}(\xb | \zb) \right] + \mathbb{E}_{q_{\phib}(\zb)}\left[ \log p(\zb) \right] \nonumber \\ 
    &\quad - \mathbb{E}_{q_{\phib}(\zb,\xb)}\left[ \log q_{\phib}(\zb | \xb) \right]
\end{align}
%
% where we let $q(\xb) = \hat{p}(\xb)$ for ease of notation so that $q_\phib(\zb,\xb) = q_{\phib}(\zb|\xb)\hat{p}(\xb)$ and we defined the aggregated posterior $q_\phib(\zb) = \int q_\phib(\zb|\xb)\hat{p}(\xb)\,\mathrm{d}\xb$. 

\todo[inline]{We can do this without all the special terminology. Just add and subtract E_{q(z)}[log q(z)].}

We can observe that the second term $\mathbb{E}_{q_{\phib}(\zb)}\left[ \log p(\zb) \right]$ is the cross-entropy $H(q_\phib(\zb),p(\zb))$ which can also be written as $D_\text{KL}(q_\phib(\zb)||p(\zb)) + H(q_\phib(\zb))$. We can also observe that the third term $-\mathbb{E}_{q_{\phib}(\zb,\xb)}\left[ \log q_{\phib}(\zb | \xb) \right]$ is the conditional entropy $H_{q_\phib(\zb,\xb)}[\zb|\xb]$ which can be factored as $H_{q_\phib(\zb)}[\zb] - I_{q_\phib(\zb,\xb)}[\zb;\xb]$ where $I_{q_\phib(\zb,\xb)}[\zb;\xb]$ is the mutual information between $\zb$ and $\xb$ under the joint distribution $q_\phib(\zb,\xb)$. 
This gives us the following decomposition of the ELBO,
%
\begin{align}
    \mathbb{E}_{\hat{p}(\xb)}\left[ \log p(\xb) \right] 
    &\geq \mathbb{E}_{q_{\phib}(\zb,\xb)}\left[ \log p_{\thetab}(\xb | \zb) \right] + H(q_\phib(\zb),p(\zb)) + H_{q_\phib(\zb,\xb)}[\zb|\xb]
\end{align}
%
% \begin{align}
%     a &= \mathbb{E}_{\xb,\zb \sim q_{\phib}(\zb|\xb)\hat{p}(\xb)}\left[ \log p_{\thetab}(\xb | \zb) \right]
% \end{align}
...
\paragraph{Deep generative models} VAEs also belong to the class of deep generative models which are models that learn a probabilistic representation of the input data and can be used to generate new samples from the same distribution. 
