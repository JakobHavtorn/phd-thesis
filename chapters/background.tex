...
Instead, VAEs amortize the cost of variational inference by parameterizing the approximate posterior with a neural network that transforms any data point $\xb$ into the parameters of the corresponding conditional distribution, $q_\phib(\zb|\xb)$. This makes it possible to efficiently infer the approximate posterior distribution for any input $\xb$. 
With amortized variational inference, the ELBO can simply be written as,
