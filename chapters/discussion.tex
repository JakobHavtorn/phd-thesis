...
%
In this thesis we studied two different approaches to speech representation learning: VAEs in \cref{chp:paper-hierarchical,chp:paper-mo?delagnostic,chp:paper-benchmarking} and self"=supervised methods in \cref{chp:paper-review}. 
...
First, VAEs are trained to maximize the ELBO which is a marginal likelihood objective conditionally independent of the latent variables $\zb$. Second, as we saw in \cref{eq:variational-autoencoder-elbo-factorized-expected}, the ELBO contains can be written to reveal that its maximization leads to minimization of the mutual information between the observed and latent variables. In the following we will discuss each of these observations.
...
\end{align}
%
The last equation follows from the law of large numbers.
% \begin{align}
...
% The first component of the ELBO is the negative reconstruction error. In other words,  𝐳  is sampled from variational posterior for a given  𝐱  and then it is stochastically reconstructed by  𝑝𝜃(𝐱|𝐳) . That is, the conditional likelihood is used to calculate the reconstruction error. This term tries to make  𝑞𝜙  as peaky as possible. If  𝑞𝜙  is non-concentrated, is is hard to learn a stochastic mapping from multiple  𝐳 's to a single  𝐱 .

% The second component of the ELBO,  𝐷KL[𝑞𝜙(𝐳)‖𝑝𝜆(𝐳)] , determines the error between the aggregated posterior and the marginal over  𝐳 (a.k.a. the \textit{prior}). Since we maximize the ELBO, and the term 𝐷KL[𝑞𝜙(𝐳)‖𝑝𝜆(𝐳)]  is with the negative sign, we want to minimize it. Thus, the ELBO tells us that the difference between  𝑞𝜙(𝐳)  and  𝑝𝜆(𝐳) should be as small as possible. This makes perfect sense because we want the aggregated posterior to assign the probability mass to the same regions as the prior.

% The last component of the ELBO,  𝕀𝑞𝜙(𝐱,𝐳)[𝐱;𝐳] , is the mutual information between  𝐱  and  𝐳  for the aggregated posterior and the empirical distribution. This term appears with the negative sing, hence, the ELBO indicates that this mutual information should be minimized. This is a very puzzling result because, in other words, there should be no stochastic dependency between  𝐳  and  𝐱 . Commonly,  𝐳  is interpreted as a representation of  𝐱 , however, optimizing the ELBO leads to a completely different outcome!  𝕀𝑞𝜙(𝐱,𝐳)[𝐱;𝐳]  is a cumbersome component amd it may be an explanation why training latent variable models with variational inference is typically hard.

% In the conclusion, the ELBO navigates the model to fit the aggregated posterior to the prior, and, on the other hand, to learn no stochastic dependency between  𝐱  and  𝐳  while maintaining a one-to-one mapping from  𝐳  to  𝐱 . This last contradiction may result in a difficult training of the model. As reported in the literature (e.g., (Alemi et al., 2018)), the choice of parameterizations of the distributions play the crucial role and could be the reason for complicating the training. For instance, taking a very powerful parameterization of the conditional likelihood,  𝑝𝜃(𝐱|𝐳) , e.g., a deep autoregressive model, gives a model that completely disregards  𝐳 .

\paragraph{How has this ever worked}
...
We can understand the reason for this by noting that both of the above scenarios are only achievable if the optimization too unconstrained. 
Specifically, the maximum likelihood estimation takes place not over all possible models, but a constrained parametric class defined by the inference network $q_\thetab(\zb|\xb) \approx p(\zb|\xb)$ and the generative model $p_\thetab(\xb|\zb)p(\zb) = p_\theta(\xb,\zb)$. This introduces a coupling between the marginal $p_{\thetab}(\xb)$ and the posterior $p_{\thetab}(\zb|\xb)$ which makes the maximum likelihood solution unlikely to correspond to completely useless representations \parencite{huszar_is_2017}. 

Similarly, maximizing the ELBO in \cref{eq:variational-autoencoder-elbo-factorized-expected (discussion copy)} by exactly minimizing the mutual information term ...
