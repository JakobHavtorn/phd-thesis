...
The studies of \cref{part:unsupervised-uncertainty-estimation} dealt with OOD detection in the context of image classification, somewhat casually ignoring the fact that this thesis is mainly interested in audio and speech data. Image classification has been the de facto standard task for benchmarking OOD detection methods

For certain image distributions, it might be possible to deem an image OOD due to the color or intensity of a single pixel. 
However, for most complex distributions we are usually interested in a test relating to more semantic properties of the data - for instance the animal class, as discussed in the introduction (\cref{subsec:understanding-uncertainty}). 

For speech, this property is similar; a single amplitude value is unlikely to provide much information about the distributional origin of a speech sample. Instead, whether a sample is OOD must be inferred from correlations and patterns found in amplitudes along the temporal dimension. 

One study \parencite{bukhsh_outofdistribution_2023} ... (OOD detection on audio)
