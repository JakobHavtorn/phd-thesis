...
\paragraph{Stop representing?}

Two key observations relating to the training of VAEs might lead to the more pessimistic view that VAEs are fundamentally unsuited for representation learning. 
First, variational autoencoders are trained to maximize the ELBO which is a marginal likelihood objective that is conditionally independent of the latent variables $\zb$. Second, as we saw in \cref{eq:variational-autoencoder-elbo-factorized-expected}, the ELBO contains a hidden \todo[inline]{Continue here!}
