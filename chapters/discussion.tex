...
Two key observations might lead to the more pessimistic view that VAEs are fundamentally unsuited for representation learning. 
First, variational autoencoders are trained to maximize the ELBO which is a marginal likelihood objective conditionally independent of the latent variables $\zb$. Second, as we saw in \cref{eq:variational-autoencoder-elbo-factorized-expected}, the ELBO contains can be written to reveal that its maximization leads to minimization of the mutual information between the observed and latent variables. In the following we will discuss each of these observations in turn.
