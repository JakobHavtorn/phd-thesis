...
Though this incident is an extreme one, it highlights the challenges of applying machine learning in real-world scenarios and underlines the need for machine learning systems to be able to accurately assess their own uncertainty and operate accordingly. 
...
At the start of this project in 2020, Corti's main product was a software system providing decision-support for emergency medical dispatchers.
The system 
% was in use in several countries, including the 1-1-2 line in Copenhagen, Denmark and the 9-1-1 line in Seattle, Washington in the United States and 
...
A reasonable machine learning pipeline for recognizing stroke might consist of an automatic speech recognition model to transcribe the conversation and a binary classifier to estimate the probability that what the transcript describes is a case of stroke. 
...
% Corti's machine learning systems are not exempt from the challenges that come with real-world applications. The development is an ongoing effort to improve performance and to ensure robustness, reliability and compliance with current regulations. This thesis 


% One of the challenges faced by Corti's systems is the need to be able to recognize and handle rare events.
% For instance, the system for emergency medical dispatchers is designed to recognize cases of cardiac arrest in emergency calls.
% Cardiac arrest is a rare event, occurring in only $0.1\%$ of emergency calls \cite{cite14}, and the system must be able to recognize it in order to provide the dispatcher with the correct protocol to follow.
% The system is trained on a dataset of calls where the dispatcher has indicated that the patient was in cardiac arrest, but the dataset is small and imbalanced, making it difficult to train a model that can reliably recognize cardiac arrest cases.
% In addition, the system must be able to recognize cardiac arrest cases in calls from different countries, where the protocols and the way dispatchers handle calls may differ.
% This is a challenging task as the system must be able to generalize to unseen data and adapt to different contexts.
...
Mutually dependent classes also complicates uncertainty estimation. Some classes are mutually exclusive, while others are not. Some classes are easily confused with each other.
}
...
\todo[inline]{Expand in this discussion of overconfidence and calibration. E.g.: Calibration is fairly well understood in the context of classification, especially binary, but is harder in structured prediction such as automated speech recognition.}
