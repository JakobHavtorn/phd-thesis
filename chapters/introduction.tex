...
\begin{figure}[t]
    ...
    \caption{Ruth Lichterman (left) and Marlyn Wescoff (middle) were two of the several female programmers of the ENIAC.} %Photo: \copyright Corbis/ Getty Images.}
    \label{fig:eniac_programmers}
\end{figure}

With the advent of electronic computers, the internet, and the widespread adoption of digital technologies, the amount of data available to decision-makers has grown exponentially. 
...
Though this incident is an extreme one, it highlights the challenges of applying machine learning in real-world scenarios and underlines the need for machine learning systems to be robust in rare, adverse or difficult situations.
...
A reasonable machine learning pipeline to assist in recognizing stroke cases on calls to emergency services might consist of an automatic speech recognition model that transcribes the conversation and a binary classifier that estimates the probability that the transcript describes a case of stroke. 
...
In practice, we are interested in the predictive uncertainty, so we need to include both aleatoric and epistemic uncertainty in the estimate. 
Since sources of epistemic uncertainty are by definition those sources not represented in the data, it is not possible to model them directly, as it is for aleatoric uncertainty. This generally makes epistemic uncertainty more difficult to model and quantify than aleatoric uncertainty. 
Some methods for estimating epistemic uncertainty include Bayesian neural networks \cite{mackay_practical_1992, neal_bayesian_1995}, Monte Carlo dropout \cite{gal_dropout_2016}, and ensembling \cite{lakshminarayanan_simple_2017}. Common to these methods is that they represent epistemic uncertainty by an implicit or explicit distribution over the model parameters. This is in contrast to aleatoric uncertainty, which is usually represented by a distribution over the data. The methods often require multiple forward passes through the model to estimate the predictive uncertainty. This makes them computationally expensive and impractical for real-time applications such as speech recognition. 
