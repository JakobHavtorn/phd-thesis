...
\chapter[supplementary material for benchmarking generative latent variable models for speech]{Supplementary material for Benchmarking generative latent variable models for speech}
...
\begin{table}[t!]
    ...
    \end{tabular}
    \vspace{2mm}
    \caption[The highest possible Gaussian log-likelihoods attainable on the TIMIT test]{The highest possible Gaussian log-likelihoods (max $\mathcal{L}$) attainable on the TIMIT test set as computed by \eqref{eq: gaussian ll with minimum variance} with different values of the minimum variance $\sigma^2_\text{min}$.}
