...
\section{Datasets}
\cref{tab:datasets-overview} lists the datasets used in the paper. We use the predefined train/test splits for the datasets.
...
In \cref{tab:hyperparameters} we specify the hyperparameters used when training our models.
...
\begin{table}[h]
    ...
    \end{tabular}
    % }
    \caption{Selection of most important hyperparameters and their setting. Convolutional kernels are square and latent dimensions are given without spatial dimensions which are given in the text. See \cref{sec:model-details} for more details.}
...
\begin{figure}[t]
    ...
    \caption{The expected inverse volume change for Gaussian Jacobians \eqref{eq:Edet} on a log-scale.}
    \label{fig:Edet}
\end{figure}
...
Assuming that the increase in layer dimension $d_{i} - d_{i-1}$ is constant, then we see that \eqref{eq:Edet} goes to zero as $d_{i}$ goes to infinity as the $\Gamma$ function grows super-exponentially to infinity.
This super-exponential growth further implies that the first layers dominate the marginal likelihood $p(\xb)$.
This is also visually evident in \cref{fig:Edet}.


\section{Derivation of the $\mathcal{L}^{>k}$ bound}
...
&= \log \int\int \frac{q(\zb_2|\xb)}{q(\zb_2|\xb)} p(\xb|\zb_1)p(\zb_1|\zb_2)p(\zb_2) \text{d}\zb_1\text{d}\zb_2 \notag \\
&= \log \int\int q(\zb_2|\xb) p(\zb_1|\zb_2) \frac{p(\xb|\zb_1)p(\zb_2)}{q(\zb_2|\xb)} \text{d}\zb_1\text{d}\zb_2 \notag \\
...
\begin{align}
    ...
    &= \log \int \frac{q(\zb_{>k}|\xb)}{q(\zb_{>k}|\xb)} p(\xb|\zb)p(\zb) \text{d}\zb \notag \\
    &= \log \int q(\zb_{>k}|\xb) p(\zb) \frac{p(\xb|\zb)}{q(\zb_{>k}|\xb)} \text{d}\zb \notag \\
    &= \log \int q(\zb_{>k}|\xb) p(\zb_{\leq k}|\zb_{>k}) p(\zb_{>k}) \frac{p(\xb|\zb)}{q(\zb_{>k}|\xb)} \text{d}\zb \notag \\
    &= \log \int q(\zb_{>k}|\xb) p(\zb_{\leq k}|\zb_{>k}) \frac{p(\xb|\zb)p(\zb_{>k})}{q(\zb_{>k}|\xb)} \text{d}\zb \notag \\
    &\ge \mathbb{E}_{p(\zb_{\leq k}|\zb_{>k})} \left[ \log q(\zb_{>k}|\xb) \frac{p(\xb|\zb)p(\zb_{>k})}{q(\zb_{>k}|\xb)} \right] \notag \\
    &\ge \mathbb{E}_{p(\zb_{\leq k}|\zb_{>k})q(\zb_{>k}|\xb)} \left[ \log \frac{p(\xb|\zb)p(\zb_{>k})}{q(\zb_{>k}|\xb)} \right] \equiv \mathcal{L}^{>k} \notag \ .
\end{align}
...
\section{The complementary $\mathcal{L}^{<l}$ bound}
...
\begin{equation}\label{eq:biva-<l}
...
% \begin{equation}
    ...
    % We can further generalize the log-likelhood ratio $LLR^{>k}$ by replacing the ELBO $\mathcal{L}$ with the bound in \eqref{eq:biva-<l} to obtain
    % \begin{equation}
        %     LLR^{>k}_{<l} = \mathcal{L}^{<l} - \mathcal{L}^{>k}.
% \end{equation}
...
% \begin{align}
    ...
    %     &- \log \left( q(\zb_1|\xb) q(\zb_2|\zb_1)\right) \right]\ .
    % \end{align}
    ...
    &+ \mathbb{E}_{q(\zb_1,\zb_2|\xb)} \left[ \log p(\zb_1|\zb_2) - \log q(\zb_1|\xb) \right] \notag \\
    &+ \mathbb{E}_{q(\zb_1,\zb_2|\xb)} \left[ \log p(\zb_2) - \log q(\zb_2|\zb_1) \right] \notag \ .
\end{align}
We can write out the expectations in order to derive the KL-divergence terms of the bottom-up ELBO:
\begin{align}
    \log p(\xb) &\ge \int \int \log p(\xb|\zb_1) \text{d}\zb_2\zb_1 \label{eq:2 layer VAE bottom up ELBO integrals} \\
    &+ \int q(\zb_1|\xb) \int q(\zb_2|\zb_1) \log \frac{p(\zb_1|\zb_2)}{q(\zb_1|\xb)} \text{d}\zb_2\zb_1 \notag \\ 
    &+ \int q(\zb_1|\xb) \int q(\zb_2|\zb_1) \log \frac{p(\zb_2)}{q(\zb_2|\zb_1)} \text{d}\zb_2\zb_1 \notag  \ .
\end{align}
...
\begin{align}
    \log p(\xb) &\ge \mathbb{E}_{q(\zb_1, \zb_2|\xb)} \left[ \log p(\xb|\zb_1) \right] \\
    &+ \mathbb{E}_{q(\zb_1|\xb)}\left[ \mathbb{E}_{q(\zb_2|\zb_1)}\left[\log \frac{p(\zb_1|\zb_2)}{q(\zb_1|\xb)}\right]\right] \notag \\
    &+ \mathbb{E}_{q(\zb_1|\xb)}\left[-D_\text{KL}[q(\zb_2|\zb_1)||p(\zb_2)]\right] \notag \ .
\end{align}
...
% \begin{align}
    ...
    \begin{align}
        \log p(\xb) &\ge \mathbb{E}_{q(\zb_1, \zb_2|\xb)} \left[ \log p(\xb|\zb_1) \right] \\ 
        &+ \mathbb{E}_{q(\zb_1,\zb_2|\xb)} \left[ \log p(\zb_2|\xb) - \log q(\zb_2|\xb) \right] \notag \\
        &+ \mathbb{E}_{q(\zb_1,\zb_2|\xb)} \left[ \log p(\zb_1|\zb_2) - \log q(\zb_1|\zb_2) \right] \notag \ .
    \end{align}
    ...
    \begin{align}
        ...
        &+ \int q(\zb_2|\xb) \log \frac{p(\zb_2|\xb)}{q(\zb_2|\xb)} d\zb_2 \notag \\ 
        &+ \int q(\zb_2|\xb) \int q(\zb_1|\zb_2) \log \frac{p(\zb_1|\zb_2)}{q(\zb_1|\zb_2)} d\zb_1\zb_2 \notag \ .
    \end{align}
    The KL-divergence terms can now easily be computed by:
    \begin{align}
        \log p(\xb) &\ge \mathbb{E}_{q(\zb_1, \zb_2|\xb)} \left[ \log p(\xb|\zb_1) \right] \\
    &- D_\text{KL}[q(\zb_2|\xb)||p(\zb_2)] \notag \\ 
    &- \mathbb{E}_{q(\zb_2|\xb)}\left[D_\text{KL}[q(\zb_1|\zb_2)||p(\zb_1|\zb_2)\right] \notag \ .
\end{align}
...
We provide additional results for a model trained on FashionMNIST in \cref{tab:additional-results-fashionmnist}, a model trained on MNIST in \cref{tab:additional-results-mnist}, a model trained on CIFAR10 in \cref{tab:additional-results-cifar} and a model trained on SVHN in \cref{tab:additional-results-svhn}.
...
\begin{table}[t]
    ...
    \begin{tabular}{llrrr}
        ...
        % \midrule
        % UniformNoise       &  $L^{>0}$         &  $1.000$  &  $1.000$  &  $0.000$  \\
        % UniformNoise       &  $L^{>1}$         &  $1.000$  &  $1.000$  &  $0.000$  \\
        % UniformNoise       &  $L^{>2}$         &  $0.965$  &  $0.957$  &  $0.055$  \\
    ...
    \end{tabular}
    % }
    \caption{Additional results for the HVAE model trained on SVHN. All results computed with 1000 importance samples.}
    \label{tab:additional-results-svhn}
\end{table}
...
\begin{table}[t]
    ...
    \begin{tabular}{llrrr}
        ...
        % \midrule
        % UniformNoise  &  $L^{>0}$    &  1.000  &  1.000  &  0.000  \\
% UniformNoise  &  $L^{>1}$    &  0.987  &  0.984  &  0.025  \\
% UniformNoise  &  $L^{>2}$    &  0.031  &  0.313  &  0.971  \\
...
CIFAR10       &  $LLR^{>1}$  &  0.469  &  0.479  &  0.835  \\
    ...
    \end{tabular}
    % }
    \caption{Additional results for the HVAE model trained on CIFAR10. All results computed with 1000 importance samples.}
    \label{tab:additional-results-cifar}
\end{table}
...
% UniformNoise                     &  $L^{>0}$  &  $1.000$  &  $1.000$  &  $0.000$ \\
% UniformNoise                     &  $L^{>1}$  &  $1.000$  &  $1.000$  &  $0.000$ \\
% UniformNoise                     &  $L^{>2}$  &  $1.000$  &  $1.000$  &  $0.000$ \\
...
MNIST                          &  $LLR^{>2}$  &  $0.515$  &  $0.507$  &  $0.792$ \\
    ...
    \end{tabular}
    % }
    \caption{Additional results for the HVAE model trained on MNIST. All results computed with 1000 importance samples.}
    \label{tab:additional-results-mnist}
\end{table}
